...
import { Pill, Syringe, FlaskConical, PenLine, Lock, ChevronLeft, ChevronRight, Check, AlertTriangle } from 'lucide-react'
import { DOSE_OPTIONS, MEDICATION_LABELS, Medication, Sex, UserProfile, WEEK_DAYS_FULL } from '../types'
import { calcIMC } from '../utils/bmiUtils'
import { DoseMode } from '../utils/medicationUtils'

interface Props {
  onComplete: (profile: UserProfile) => void
}

const MEDICATION_ICONS: Record<Medication, React.ReactNode> = {
  tirzepatida: <Pill         size={18} strokeWidth={2} />,
  semaglutida: <Syringe      size={18} strokeWidth={2} />,
  ozempic:     <Syringe      size={18} strokeWidth={2} />,
  wegovy:      <Syringe      size={18} strokeWidth={2} />,
  mounjaro:    <Pill         size={18} strokeWidth={2} />,
  retatrutida: <FlaskConical size={18} strokeWidth={2} />,
  outro:       <PenLine      size={18} strokeWidth={2} />,
}

const MEDICATIONS = (Object.keys(MEDICATION_LABELS) as Medication[]).map(value => ({
  value, label: MEDICATION_LABELS[value], icon: MEDICATION_ICONS[value],
}))

const STEPS = [
  { id: 1, title: 'Bem-vindo ao Minha Tize',  subtitle: 'Vamos configurar seu perfil em 5 passos rápidos.' },
    ...
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '32px' }}>
          <img
            src="/LogoPng.png" alt="Minha Tize"
            style={{ width: '46px', height: '46px', borderRadius: '12px', flexShrink: 0, objectFit: 'contain' }}
          />
          <span style={{ fontWeight: 800, fontSize: '16px', color: 'var(--text-primary)' }}>Minha Tize</span>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                  {MEDICATIONS.map(m => (
                    <button key={m.value} onClick={() => selectMedication(m.value)}
                      style={{
                      ...
                      {m.value === 'retatrutida' && (
                        <span className="badge badge-warm" style={{ fontSize: '9px' }}>investigacional</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              {form.medication === 'outro' && (
                <div>
                  ...
                  <input type="text" className="input-field" placeholder="Digite o nome do medicamento"
                    value={form.medicationName} onChange={e => set('medicationName', e.target.value)}
              ...
              <div>
                ...
                {form.medication !== 'retatrutida' && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '7px', marginBottom: '7px' }}>
                    {DOSE_OPTIONS.map(d => (
                      <button key={d}
                        style={selBtn(form.doseMode === 'fixed' && form.fixedDose === d)}
                        onClick={() => setForm(f => ({ ...f, doseMode: 'fixed', fixedDose: d }))}>
                        {d}mg
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px' }}>
                  <button style={selBtn(form.doseMode === 'custom')} onClick={() => set('doseMode', 'custom')}>
                    Outra dose
                  </button>
                  <button style={selBtn(form.doseMode === 'unknown')} onClick={() => set('doseMode', 'unknown')}>
                    Não sei informar
                  </button>
                </div>
                {form.doseMode === 'custom' && (
                  <div style={{ marginTop: '10px' }}>
                    ...
                    <input type="number" className="input-field" placeholder="Ex: 6"
                      value={form.customDose} onChange={e => set('customDose', e.target.value)}
              ...
              <div>
                <label className="label-base">Apresentação do produto (opcional)</label>
                <p style={{ fontSize: '11px', color: 'var(--text-muted)', margin: '-4px 0 8px' }}>
                  Ex.: 15 mg / 0,6 mL — não é obrigatório e não trava o cadastro.
                </p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <input type="number" className="input-field" placeholder="Concentração (mg)"
                    value={form.presentationMg} onChange={e => set('presentationMg', e.target.value)} min={0.1} step={0.1} />
                  <input type="number" className="input-field" placeholder="Volume (mL)"
                    value={form.presentationMl} onChange={e => set('presentationMl', e.target.value)} min={0.01} step={0.01} />
                </div>
              </div>
            </div>
          )}
              ...
              {form.applicationDay !== undefined && (
                <div style={{ padding: '12px 14px', background: 'var(--primary-light)', borderRadius: '11px', border: '1px solid rgba(37,99,235,0.15)' }}>
                  <p style={{ fontSize: '13px', color: 'var(--primary)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    Você aplica toda {WEEK_DAYS_FULL[form.applicationDay]}. O Minha Tize vai acompanhar isso pra você.
                  </p>
                </div>
              )}
            </div>
